...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Xumchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light Xumchain handshake failed", "err", err)
		return err
	}
	...
	for {
		...
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Xumchain message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Xumchain message arrived", "code", msg.Code, "bytes", msg.Size)
