...
// nodeDockerfile is the Dockerfile required to run an Xumchain node.
var nodeDockerfile = `
...
// an Xumchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Xumchain node container to a remote machine via SSH,
